...
extern BOOL handle_set_configuration(BYTE cfg);  // See 4 lines below. exact same declaration. duplicate?
...
extern BOOL handle_set_configuration(BYTE cfg);  // See 4 lines above. duplicate declaration??
...
void handle_get_descriptor() {
    ...
    //    
    //    Without the below line, the code results in device not getting enumerated properly.
    //    resulting in failing of lsusb -v (verbose mode) and also numerous kernel debug messages in dmesg
    //    and also obviously the mplayer or guvcview, both result in same kernel debug messages in dmesg something like this
    //          reset high-speed USB device number 13 using ehci-pci
    //          usb 1-1.1.6: device descriptor read/64, error 2
    //          usb 1-1.1.6: device descriptor read/64, error 2
    //    
    //    and in the end the device stops getting listed in the pc, with error like this:
